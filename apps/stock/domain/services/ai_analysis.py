...
from apps.stock.domain.models.stock import ChartPoint, StockQuote
...
def _provider_enabled(provider: str) -> bool:
    """관리자 화면의 "사용" 토글 -- 꺼두면 activate/자동선택 대상에서 제외된다
    (aura의 API key 관리 화면과 항목을 통일하면서 2026-07-22에 추가)."""
    return {
...
def _resolve_provider() -> Optional[str]:
    if config.AI_PROVIDER:
        if (
            config.AI_PROVIDER in _PROVIDER_ORDER
            and _provider_key(config.AI_PROVIDER)
            and _provider_enabled(config.AI_PROVIDER)
        ):
            return config.AI_PROVIDER
        return None  # 명시적으로 지정된 provider가 키/사용설정을 만족하지 않음

    for provider in _PROVIDER_ORDER:
        if _provider_key(provider) and _provider_enabled(provider):
            return provider
    return None
...
def _build_prompt(quote: StockQuote) -> str:
    ...
    return (
        "아래 제공 데이터만 사용하고 숫자를 새로 추정하지 마세요. 다음 제목과 순서를 정확히 지키세요:\n"
        "[사실 요약]\n[사용 지표/기간]\n[긍정 요인]\n[위험 요인]\n[확인할 일정]\n[면책]\n"
        "확인할 일정 데이터가 없으면 '제공된 일정 없음'이라고 쓰고, 매수·매도 추천은 하지 마세요.\n\n"
        ...
        f"최근 1개월 추세: {trend}\n"
        "면책 고정 문구: 이 내용은 정보 제공 목적이며 투자 권유가 아닙니다.\n"
    )
