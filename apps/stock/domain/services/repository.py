...
from apps.stock.domain.models.stock import ChartPoint, StockQuote, StockReference
...
def _fetch_single_quote_sync(
    ...
    if include_chart:
        for ts, close_value in closes.items():
            ...
            if close_f is None:
                continue
            volume = hist.loc[ts].get("Volume") if "Volume" in hist.columns else None
            chart_points.append(ChartPoint(timestamp=int(ts.timestamp()), close=close_f, volume=int(volume) if volume is not None and not math.isnan(float(volume)) else None))

    details = {}
    financial_trends = []
    if include_chart:
        try:
            details = ticker.info or {}
        except Exception:
            details = {}
        try:
            financials = ticker.financials
            revenue = financials.loc["Total Revenue"] if "Total Revenue" in financials.index else None
            operating = financials.loc["Operating Income"] if "Operating Income" in financials.index else None
            for column in list(financials.columns)[:5][::-1]:
                financial_trends.append({"period": str(getattr(column, "date", lambda: column)()), "revenue": _safe_float(revenue.get(column)) if revenue is not None else None, "operating_income": _safe_float(operating.get(column)) if operating is not None else None})
        except Exception:
            financial_trends = []
    ...
    return StockQuote(
        ...
        fifty_two_week_low=_safe_float(details.get("fiftyTwoWeekLow")),
        fifty_two_week_high=_safe_float(details.get("fiftyTwoWeekHigh")),
        trailing_pe=_safe_float(details.get("trailingPE")),
        price_to_book=_safe_float(details.get("priceToBook")),
        regular_market_price=_safe_float(details.get("regularMarketPrice")),
        pre_market_price=_safe_float(details.get("preMarketPrice")),
        post_market_price=_safe_float(details.get("postMarketPrice")),
...
async def fetch_quote_detail(reference: StockReference, period: str = "1mo") -> StockQuote:
    """Fetch a 1-month history (used for the detail view + price chart)."""
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(
        _executor, _fetch_single_quote_sync, reference, period, True
    )
