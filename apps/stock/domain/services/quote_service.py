...
from apps.stock.domain.models.stock import MarketCategory, StockQuote, StockReference
from apps.stock.domain.services import repository
...
async def get_quote_detail(
    ...
    quote = await repository.fetch_quote_detail(reference, period)

    async with _lock:
        _detail_cache[cache_key] = _CacheEntry(
            value=quote,
